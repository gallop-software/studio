 ...
 */
export function useStreamingOperation(deps: StreamingOperationDeps): StreamingOperationResult {
  const { setShowProgress, setProgressTitle, setProgressState, triggerRefresh } = deps
  ...
  const execute = useCallback(async (config: StreamingOperationConfig) => {
    ...
    const operationType = endpoint.split('/').pop()?.replace('-stream', '') || 'op'
    const operationId = `${operationType}-${Date.now()}-${Math.random().toString(36).slice(2)}`
            ...
            try {
              ...
              if (data.type === 'start') {
                ...
                setProgressState({
                  ...
                  total: data.total || 0,
                  percent: data.percent || Math.round(((data.current || 0) / (data.total || 1)) * 100),
                  status: 'processing',
                ...
                setProgressState({
                  ...
                  total: processed,
                  percent: 100,
                  status: wasCancelled ? 'stopped' : (data.errors && data.errors > 0 ? 'error' : 'complete'),
                  message: data.message,
                  processed,
                  errors: data.errors,
